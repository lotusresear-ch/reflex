...
// Mock Algebra Factory for testing
...
contract BasePluginV3FactoryTest is Test {
    ...
    function setUp() public {
        ...
        // Create mock pool
        pool = new MockPool(address(token0), address(token1), address(algebraFactory));
        algebraFactory.setPoolByPair(address(token0), address(token1), address(pool));
        
        // Create mock Reflex router
        reflexRouter = new MockReflexRouter(admin);
    ...
    function testBeforeCreatePoolHook() public {
        ...
        address plugin = factory.beforeCreatePoolHook(
            mockPool,
            address(token0),
            address(token1),
            address(0),
    ...
    function testPluginCreatedWithZeroReflexRouter() public {
        ...
        MockToken token2 = MockToken(TestUtils.createMockToken("Token2", "TK2", 1000000 * 10**18));
        MockToken token3 = MockToken(TestUtils.createMockToken("Token3", "TK3", 1000000 * 10**18));
        MockPool pool2 = new MockPool(address(token2), address(token3), address(algebraFactory));
    ...
    function testPluginCreatedAfterReflexRouterChange() public {
        ...
        MockReflexRouter newRouter = new MockReflexRouter(alice);
        vm.prank(admin);
    ...
    function testMultiplePluginCreation() public {
        ...
        MockPool pool2 = new MockPool(address(token2), address(token3), address(algebraFactory));
        MockPool pool3 = new MockPool(address(token2), address(token4), address(algebraFactory));
    ...
    function testConstants() public {
        assertEq(
            factory.ALGEBRA_BASE_PLUGIN_FACTORY_ADMINISTRATOR(),
            keccak256("ALGEBRA_BASE_PLUGIN_FACTORY_ADMINISTRATOR")
        );
    }
