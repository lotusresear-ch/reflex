...
contract CallbackTestV2Pool {
    ...
    function swap(uint256 amount0Out, uint256 amount1Out, address to, bytes calldata data) external {
        ...
        if (data.length > 0) {
            (bool success,) = msg.sender.call(abi.encodeWithSignature("swap(uint256,uint256,bytes)", amount0Out, amount1Out, data));
...
contract CallbackTestV3Pool {
    ...
    function swap(
        ...
        if (data.length > 0) {
            (bool success,) = msg.sender.call(abi.encodeWithSignature("swap(int256,int256,bytes)", amount0, amount1, data));
            require(success, "Callback failed");
        }
    }
}
